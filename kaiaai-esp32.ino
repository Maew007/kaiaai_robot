...
/*
 * มีแก้ไขบรรทัดที่ 481 และ 482 เพื่อกำหนด SSID และ รหัสผ่าน
 *      แก้ไขจาก #include "LDS_all_models.h" เป็น #include "lds_all_models.h" 
 */
...
void twist_sub_callback(const void *msgin) {
  ...
  Serial.print("linear.x");Serial.println(target_speed_lin_x);
  Serial.print("msg->angular.z");Serial.println(target_speed_ang_z);
          ...
          if (strcmp(model, "YDLIDAR X3") == 0) {
            ...
            if (strcmp(model, "YDLIDAR X3 PRO") == 0) {
              ...
              if (strcmp(model, "DELTA 2G") == 0) {
                ...
                Serial.print(" not recognized, new LDS_DELTA_2G");
              } else {
